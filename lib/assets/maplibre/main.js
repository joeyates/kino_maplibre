...
export function init(ctx, data) {
  ...
  map.on("click", (e) => {
    ctx.pushEvent("click", e.lngLat);
  })

  map.on("move", (e) => {
    const bounds = boundsToObject(map.getBounds());
    ctx.pushEvent("move", { bounds });
  })

  map.on("rotate", (e) => {
    const bounds = boundsToObject(map.getBounds());
    ctx.pushEvent("rotate", { bounds });
  })

  map.on("zoom", (e) => {
    const bounds = boundsToObject(map.getBounds());
    ctx.pushEvent("zoom", { bounds });
  })
  ...
  function boundsToObject(bounds) {
    ...
    return { sw: { lng: sw[0], lat: sw[1] }, ne: { lng: ne[0], lat: ne[1] } };
  }
}
